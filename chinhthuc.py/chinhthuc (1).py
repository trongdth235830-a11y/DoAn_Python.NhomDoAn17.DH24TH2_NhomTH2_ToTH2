...
def qlphuongtien(parent):
    ...
    sticky="w") 
    entry_matuyen = tk.Entry(frame_info, width=15) 
    entry_matuyen.grid(row=0, column=3, padx=5, pady=5, sticky="w") 
...
def dattour(parent):
    ...
    def dat_tour():
        ...
        if conn:
            ...
            try:
                cursor.execute(
                    "INSERT INTO dattour (makhachhang, matuyen, ngaydat, songuoi) VALUES (%s, %s, %s, %s)",
                    (makhachhang, matuyen, ngaydat, songuoi)
                )
                conn.commit()
